...
ngantri = "https://telegra.ph/file/c41c41a1b48e64fea51c9.jpg"
...
async def play(event):
    ...
    if (
        ...
        botman = await event.reply("جاري معالجة البيانات انتظر قليلا . . .")
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                caption = f"- **تم الاضافه الى قائمة التشغيل»** `#{pos}`\n\n**⌔ العنوان:** [{songname}]({url})\n**⌔ المدة:** `{duration}`\n⌔ **الطب الى:** {from_user}"
                await botman.delete()
...
async def vc_playlist(event, perm):
    ...
    if chat_id in QUEUE:
        ...
        await event.reply("**مفيش حاجه شغاله اصلا**")
...
async def leavevc(event, perm):
    razan = await event.reply("- هغادر اهو اصبر شويه")
    chat_id = event.chat_id
    from_user = vcmention(event.sender)
    if from_user:
        try:
...
async def vc_resume(event, perm):
    ...
    if chat_id in QUEUE:
        ...
        await event.reply(event, "**مفيش حاجه شغاله اصلا**")
